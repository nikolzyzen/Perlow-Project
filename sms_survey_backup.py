...
<body>
    <div class="container">
        <div class="header">
            <h1>SMS Survey System</h1>
            <p>Track wellbeing and collect meaningful insights through daily SMS surveys</p>
        </div>
        ...
        <div class="action-card">
            <div class="action-header">
                ...
                <div class="action-title">Send Test Survey</div>
            </div>
            <form method="POST" action="/admin/test-sms">
        ...
        <div class="action-card">
            ...
            <form method="POST" action="/admin/add-user">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="user_phone" name="phone_number" class="form-control" 
                           placeholder="Enter phone number (e.g., +1234567890)" required>
                </div>
        ...
        <div class="action-card">
            <div class="action-header">
                <div class="action-icon">🎯</div>
                <div class="action-title">Add New Campaign</div>
            </div>
            <form method="POST" action="/admin/add-campaign">
                <div class="form-group">
                    <label for="campaign_name">Campaign Name</label>
                    <input type="text" id="campaign_name" name="name" class="form-control" 
        ...
        <div class="action-card">
            ...
            <div class="nav-grid">
                ...
                <a href="/admin/responses" class="btn btn-secondary">View Responses</a>
                <a href="/admin/campaigns" class="btn btn-secondary">Manage Campaigns</a>
            </div>
        </div>
    </div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Users - SMS Survey System</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .header h1 {{ font-size: 2.5rem; font-weight: 700; color: white; margin-bottom: 16px; }}
        .back-btn {{ display: inline-block; padding: 12px 24px; background: white; color: #667eea; text-decoration: none; border-radius: 8px; margin-bottom: 30px; }}
        .content {{ background: white; border-radius: 16px; padding: 40px; box-shadow: 0 8px 32px rgba(0,0,0,0.1); }}
        .user-card {{ background: #f8f9fa; border-radius: 12px; padding: 20px; margin-bottom: 16px; border-left: 4px solid #667eea; }}
        .user-name {{ font-weight: 600; color: #333; margin-bottom: 8px; }}
        .user-phone {{ color: #666; font-size: 0.9rem; }}
        .user-status {{ display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; margin-top: 8px; }}
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Responses - SMS Survey System</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{ font-family: 'Inter', sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; color: #333; }}
        ...
        .back-btn {{ display: inline-block; padding: 12px 24px; background: white; color: #667eea; text-decoration: none; border-radius: 8px; margin-bottom: 30px; }}
        .content {{ background: white; border-radius: 16px; padding: 40px; box-shadow: 0 8px 32px rgba(0,0,0,0.1); }}
        .response-card {{ background: #f8f9fa; border-radius: 12px; padding: 20px; margin-bottom: 16px; border-left: 4px solid #667eea; }}
        .response-header {{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }}
        .response-date {{ color: #666; font-size: 0.9rem; }}
        .response-ratings {{ display: flex; gap: 20px; margin-bottom: 12px; }}
        .rating {{ text-align: center; }}
        .rating-value {{ font-size: 1.5rem; font-weight: 700; color: #667eea; }}
        .rating-label {{ font-size: 0.8rem; color: #666; }}
        .influence {{ color: #333; font-style: italic; }}
    </style>
</head>
<body>
    <div class="container">
        ...
        <a href="/admin/" class="back-btn">← Back to Dashboard</a>
        <div class="content">
            <h2>All Responses ({len(responses)})</h2>
            {"".join([f'''
            <div class="response-card">
                <div class="response-header">
                    <strong>User ID: {response.user_id}</strong>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Campaigns - SMS Survey System</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{ font-family: 'Inter', sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; color: #333; }}
        ...
        .back-btn {{ display: inline-block; padding: 12px 24px; background: white; color: #667eea; text-decoration: none; border-radius: 8px; margin-bottom: 30px; }}
        .content {{ background: white; border-radius: 16px; padding: 40px; box-shadow: 0 8px 32px rgba(0,0,0,0.1); }}
        .campaign-card {{ background: #f8f9fa; border-radius: 12px; padding: 20px; margin-bottom: 16px; border-left: 4px solid #667eea; }}
        .campaign-name {{ font-weight: 600; color: #333; margin-bottom: 8px; font-size: 1.2rem; }}
        .campaign-dates {{ color: #666; font-size: 0.9rem; margin-bottom: 8px; }}
        .campaign-status {{ display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; }}
        .status-active {{ background: #d4edda; color: #155724; }}
        .status-inactive {{ background: #f8d7da; color: #721c24; }}
    </style>
</head>
<body>
    <div class="container">
        ...
        <a href="/admin/" class="back-btn">← Back to Dashboard</a>
        <div class="content">
            <h2>All Campaigns ({len(campaigns)})</h2>
            {"".join([f'''
            <div class="campaign-card">
                <div class="campaign-name">{campaign.name}</div>
                <div class="campaign-dates">
                    {campaign.start_date.strftime('%Y-%m-%d')} to {campaign.end_date.strftime('%Y-%m-%d')}
                </div>
                <span class="campaign-status {'status-active' if campaign.is_active else 'status-inactive'}">
